...
**프로젝트 코드명:** Ieum *(구 Wayfinder — §21 참조)*
...
| 10 | 프로젝트명 Wayfinder | **Ieum** | "Wayfindr" = ITU-T F.921 표준 보유 실존 조직. 혼동·상표 위험 (§21) |
...
**확정: `Ieum`.** repo·org 이름이고 상표 충돌이 없다. 위 표는 검토한 후보를 기록으로 남긴 것이며,
`Bearing`은 채택하지 않았다 — 다만 `RouteSegment.bearing`은 방위각을 뜻하는 공간 용어라 필드명으로 남는다.
